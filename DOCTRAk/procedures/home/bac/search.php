...
function NewDocument($docid,$docdetail,$prcost,$docDate)
{
   ...
   global $DB_HOST, $DB_USER,$DB_PASS, $BD_TABLE,$KEY; 
    ...
    $validDate=date_create($docDate);
    $validDate= date_format($validDate,"Y/m/d");
    //$sqlToDate=STR_TO_DATE($validDate,"%m/%d/%Y");
...
//            pr_date,fk_bacdocument_template) VALUES ("'.$docid.'","'.mysqli_real_escape_string($con, str_replace(array("'",'"'), '' , $docdetail)).'",'.$prcost.',"'.$_SESSION['OFFICE'].'","'.$_SESSION['security_name'].'","'.$validDate.'",'.$template_id.')';

 
		 $sql="INSERT INTO bacdocumentlist (bacdocument_id, bacdocumentdetail, prcost, fk_officename_bacdocumentlist, fk_security_username, pr_date,fk_bacdocument_template) VALUES ('".$docid."','".mysqli_real_escape_string($con, str_replace(array('"',"'"), '' , $docdetail))."',".$prcost.",'".$_SESSION['OFFICE']."','".$_SESSION['security_name']."','".$validDate."','".$template_id."')";

		log_audit($KEY,$sql,'New Bac Document',$_SESSION['security_name']);
//		echo $query;
//    die(); 
    $result = mysqli_query($con, $sql);
    if(!$result) { 
...
//    log_audit($KEY,$query,'New Bac Document',$_SESSION['security_name']);
...
//        else 
//        {
            $query3="INSERT INTO bacdocumentlist_tracker (fk_bacdocumentlist_id, sortorder, activity, expire_days) VALUES ('".$docid."', ".$rows2['sortorder'].", '".$rows2['activity']."', ".$rows2['max_day'].")";
//        }
//        
//        echo $query3;
//        echo $countz.'<br>';
...
//        echo $query3.'<br>';
				...
				//log_audit($KEY,'','Rollback',$_SESSION['security_name']);
				log_audit($KEY,$query3,'New Bac Document',$_SESSION['security_name']);
    }
...
function newEDIT($primKey,$docid,$docdetail,$prcost,$docDate)
{
    ...
    log_audit($KEY,$sqlString,'Edit Bac Document',$_SESSION['security_name']);
    if($flag)
    {
        mysqli_commit($con);
...
function NewDelete($docid)
{
    ...
    log_audit($KEY,$sqlString,'Delete Bac Document',$_SESSION['security_name']);
