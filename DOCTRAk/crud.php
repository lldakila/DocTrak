...
{
    global $DB_HOST, $DB_USER,$DB_PASS, $BD_TABLE,$_tracker_id,$KEY;
    $cons=mysqli_connect($DB_HOST,$DB_USER,$DB_PASS,$BD_TABLE);
...
{
    global $DB_HOST, $DB_USER,$DB_PASS, $BD_TABLE,$_tracker_id,$cons,$KEY;
    $cons=mysqli_connect($DB_HOST,$DB_USER,$DB_PASS,$BD_TABLE);
    $docID=mysqli_real_escape_string($cons,$document_id);
    mysqli_autocommit($cons,FALSE);
    if (mysqli_connect_error())
...
{
    global $DB_HOST, $DB_USER,$DB_PASS, $BD_TABLE,$KEY,$_tracker_id;
    $con=mysqli_connect($DB_HOST,$DB_USER,$DB_PASS,$BD_TABLE);
    $document_id=mysqli_real_escape_string($con,$docID);
    $sql='SELECT distinct documentlist.document_id, documentlist.document_title as title,document_type.description as type,office.office_description as office, 
...
//        $sql='SELECT distinct documentlist.document_id, documentlist.document_title as title,document_type.description as type,office.office_description as office, 
//        document_template.template_name as template, documentlist_tracker.forrelease_comment as forreleasecomment FROM documentlist JOIN
//        document_type ON documentlist.fk_documenttype_id = document_type.documenttype_id JOIN
//        document_template ON documentlist.fk_template_id = document_template.template_id JOIN
//        office on documentlist.fk_office_name_documentlist = office.office_name JOIN
//        documentlist_tracker on documentlist.document_id = documentlist_tracker.fk_documentlist_id
//        WHERE documentlist.document_id = "'.$document_id.'" ';
//    echo $sql;
//    die();
...
//    die();
    ...
    echo "
     ...
     ";
     	...
     	if ($_POST['module']=='searchReleaseDocument')
     	{
     			if (getComment($_tracker_id)!=null)
     			{
     				//echo $resultSet['forreleasecomment'];
     				
     				
     				echo '<script> $("#DocComment").val("'.getComment($_tracker_id).'");</script>';
...
//////////////////////////////////////////
    ...
    $con=mysqli_connect($DB_HOST,$DB_USER,$DB_PASS,$BD_TABLE);	
		...
		$sql="insert into feedback(message,security_name) values ('".$feedbackMsg."','".$_SESSION['usr']."')";
//		echo $sql;
//		die();
		if(mysqli_query($con,$sql))
		{
			...
			$KEY=get_key();
			log_audit($KEY,$sql,'Feedback',''.$_SESSION['security_name'].'');
		}
		else
		{
