...
function NewDocument()
{
    ...
    $flag=true;
        ...
        //LOG
            ...
            $query="SELECT  fk_office_name,SORT FROM template_list WHERE fk_template_id= '".$_POST['docTemplate']."' order by sort asc";
            $RESULT2=mysqli_query($cons,$query);

            while ($row = mysqli_fetch_array($RESULT2))
            {
                //INSERT OFFICES/TEMPLATE LIST AT DOCUMENTLIST_TRACKER
                $query="INSERT INTO documentlist_tracker(OFFICE_NAME,SORTORDER,fk_documentlist_id) VALUES ('".$row['fk_office_name']."','".$row['SORT']."','".$_POST['docId']."')";
...
//	die();
    ...
    {
        echo "Document ID does not exist. Please try again.<br>";
         $flag=false;
    }
...
//       die();
    ...
    {
        echo "Document ID does not exist.<br>";
         $flag=false;
    }
}
...
function ForReleaseDocument($document_id)
{
    ...
    if(!InsertHistory($docID,$_SESSION['OFFICE'],'Document ready for release',$_POST['docuComment'],'Processed by '.$_SESSION['security_name']))
        ...
        {
                mysqli_commit($cons);
                echo "Document ready to release.";
        }
        else
        {
                mysqli_rollback($cons);
...
function RetrieveBarcodeInfo($document_id)
{
    ...
    $sql="SELECT received_val,released_val,forrelease_val,office_description FROM documentlist_tracker JOIN office on documentlist_tracker.office_name=office.office_name 
    		WHERE fk_documentlist_id = '".$document_id."' ORDER BY sortorder ASC";
    $result=mysqli_query($con,$sql);
    
    while($resultSet=mysqli_fetch_array($result))
    {
    	if (($resultSet['received_val']==1)AND($resultSet['received_val']!=1))
    	{
    		$NameOfOffice="Document must be released first at ".$resultSet['office_description'].".";
		 		mysqli_close($con);
   			return $NameOfOffice;
    	}
    	elseif (($resultSet['received_val']!=1)AND($resultSet['received_val']!=1))
    	{
